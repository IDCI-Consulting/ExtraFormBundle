...
function filterObject(e,r){var t={};for(var n in e)e.hasOwnProperty(n)&&r(e[n])&&(t[n]=e[n]);return t}function sortObject(e,r,t){const n={};"undefined"==typeof t&&(t=!0),"undefined"==typeof r&&(r=[]);for(var a=0,o=r.length;a<o;a++){var c=r[a];n[c]=e[c]}if(t)Object.keys(e).sort().forEach(function(t){r.indexOf(t)===-1&&(n[t]=e[t])});else for(var c in e)e.hasOwnProperty(c)&&r.indexOf(c)===-1&&(n[c]=e[c]);return n}function generateUniqueId(){return Math.random().toString(36).substr(2,9)}function getLastKey(e){var r=Object.keys(e),t=r.length-1;return r[t]}function hashCode(e){var r,t=0;if(0===e.length)return t;for(var n=0;n<e.length;n++)r=e.charCodeAt(n),t=(t<<5)-t+r,t|=0;return t}function createAttributeMapObject(e){for(var r,t=e.attributes,n={},a=0,o=t.length;a<o;a++)r=t[a],n[r.nodeName]=r.nodeValue;return n.value=e.value,n}function createBootstrapModal(e,r,t,n,a,o){var c=o?o:"";return'<div id="'+r+"-"+e+'" class="editor-modal modal fade '+t+" "+r+'"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="close" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 class="modal-title">'+n+'</h4></div><div class="modal-body">'+a+'</div><div class="modal-footer">'+c+"</div></div></div></div>"}function colorEmptyRequiredInputs(e,r){function t(e){e.val()?e.css({"border-color":"#cccccc","background-color":"#ffffff"}):e.css({"border-color":"#c9302c","background-color":"#f3d9d9"})}var n="."+r+' input[required="required"]';$(document).on("change",n,function(){t($(this))});var a=document.getElementById(e),o={childList:!0,characterData:!0,subtree:!0},c=new MutationObserver(function(e){e.forEach(function(e){var a=[];a=$(e.target).hasClass(r)?$(e.target).find('input[required="required"]'):$(e.target).find(n),a.each(function(){t($(this))})})});c.observe(a,o)}function transformRawToJson(e){function r(e){return e.replace(/\\/g,"\\\\").replace(/"/g,'\\"')}function t(e){var r=e.replace(/([^\\])"/g,'$1\\"').replace(/\r?\n|\r/g," ").replace(/ {2,}/g," ");return r}var n=/{{(.*)?}}/g,a=/\[{%([\s\S]*.*)%}\]/g,o=/("([\w.|]*)"): (\[{%([\s\S]*.*)%}\])/g;return e.replace(n,r).replace(a,t).replace(o,'$1: "$3"')}function transformJsonToRaw(e){function r(e){return e.replace(/\\\\/g,"\\").replace(/\\"/g,'"')}function t(e){var r=e.replace(/\\"/g,'"').replace(/\\\\'/g,"\\'");return r.substring(1,r.length-1)}var n=/{{(.*)?}}/g,a=/"\[{%(.*)%}\]"/g;return e.replace(n,r).replace(a,t)}
...
var jsonOptionMixin={mixins:[optionMixin],data:function(){return{classes:""}},created:function(){"object"==typeof this.value&&(this.data=transformJsonToRaw(JSON.stringify(this.value,null,4)),this.setJsonAttemptClass(this.data))},watch:{value:{handler:function(t){"object"==typeof t&&(this.data=transformJsonToRaw(JSON.stringify(t,null,4)),this.setJsonAttemptClass(this.data))}}},methods:{onOptionValueChanged:function(t){this.updateOption(t),this.setJsonAttemptClass(t)},setJsonAttemptClass:function(t){if(0===t.indexOf("{{")||0!==t.indexOf("{")&&0!==t.indexOf("["))this.classes="";else try{JSON.parse(transformRawToJson(t)),this.classes="fa fa-check success feedback"}catch(s){this.classes="fa fa-exclamation-circle warning feedback"}}}};
...
var textareaOption={template:'<div class="form-group"><label :for="name">{{ name }}<i :class="classes" aria-hidden="true"></i></label><textarea class="form-control" :required="option.options.required" :value="data" @input="onOptionValueChanged($event.target.value)" :name="name"></textarea></div>',mixins:[jsonOptionMixin]};
var choiceOption={template:'<div class="form-group"><label :for="name">{{ name }}</label><select class="form-control" :required="option.options.required" v-model="data" :name="name" ><option :value="key" v-for="(choice, key) in option.options.choices">{{ choice }}</option></select></div>',mixins:[optionMixin],watch:{value:{handler:function(e){this.data=e},deep:!0},data:{handler:function(e){this.updateOption(e)},deep:!0}}};
var textOption={template:'<div class="form-group"><label :for="name">{{ name }}<i :class="classes" aria-hidden="true"></i></label><input class="form-control" :required="option.options.required" :value="data" @input="onOptionValueChanged($event.target.value)" type="text" :name="name"></div>',mixins:[jsonOptionMixin]};
var numberOption={template:'<div class="form-group"><label :for="name">{{ name }}</label><input class="form-control" :required="option.options.required" :value="data" @input="updateOption($event.target.value)" type="text" :name="name"></div>',mixins:[optionMixin]};
var integerOption={template:'<div class="form-group"><label :for="name">{{ name }}</label><input class="form-control" :required="option.options.required" :value="data" @input="updateOption($event.target.value)" type="number" :name="name"></div>',mixins:[optionMixin]};
var checkboxOption={template:'<div class="form-group"><label for="name">{{ name }}</label><input :required="option.options.required" type="checkbox" :name="name" :checked="data" @click="updateOption($event.target.checked)"> </div>',mixins:[optionMixin]};
