...
var editorAdvancedField={template:'<div :class="activeClass" @click="setActiveClass(index)"><button @click.prevent="removeField(index)" aria-label="Close" class="close"><span aria-hidden="true">×</span></button><i class="fa fa-arrows-alt"></i><i v-if="configuredTypesEditionAllowed" title="Save this field as a configured type" @click="openSaveModal" class="fa fa-floppy-o"></i><strong>{{ field.extra_form_type }}</strong><i :class="getFontAwsomeIconClass(field.icon, field.extra_form_type)" aria-hidden="true"></i><br><span>Name:<input class="field-name-input" type="text" v-model="field.name" pattern="/^([a-z][0-9])+$/" /></span><modal v-if="modal.show"><h3 slot="header">Save this configured field<button @click="closeSaveModal" type="button" class="close" aria-label="Close">&times;</button></h3><div slot="body" v-html="modal.content"></div><div slot="footer"><div v-if="modal.type == \'save\'"><button @click="saveConfiguredType(field)" type="button" class="extra-btn" aria-label="Save">Save</button></div><div v-if="modal.type == \'put\'"><button @click="updateConfiguredType(field)" type="button" class="extra-btn" aria-label="Save">Update</button></div></div></div></modal></div>',props:["field","index"],data:function(){return{modal:null}},created:function(){this.setInitialSaveModal()},computed:{activeClass:function(){var t=this.field.active?"active":"inactive";return"field "+t},configuredTypesEditionAllowed:function(){return this.$store.getters.configuredTypesEditionAllowed}},mixins:[fontAwesomeIconMixin,rawMixin],watch:{field:{handler:function(){this.modal.content='<div><i class="'+this.getFontAwsomeIconClass(this.field.icon)+'" aria-hidden="true"></i></div><div>Type: <strong>'+this.field.extra_form_type+"</strong></div><div>Name: <strong>"+this.field.name+"</strong></div>"},deep:!0}},methods:{setInitialSaveModal:function(){this.modal={show:!1,type:"save",content:'<div><i class="'+this.getFontAwsomeIconClass(this.field.icon)+'" aria-hidden="true"></i></div><div>Type: <strong>'+this.field.extra_form_type+"</strong></div><div>Name: <strong>"+this.field.name+"</strong></div>"}},openSaveModal:function(){this.modal.show=!0},closeSaveModal:function(){this.modal.show=!1,this.setInitialSaveModal()},transformFieldToType:function(t){var e=JSON.parse(JSON.stringify(t));return e.extra_form_options=e.options,delete e.options,e.extra_form_constraints=e.constraints,delete e.constraints,e.form_type=e.extra_form_type,delete e.extra_form_type,e},saveConfiguredType:function(t){var e=t.name,i=this.createExtraFormRawRecursively([t]),o=this.transformFieldToType(i[t.name]),s=this.$store.getters.postConfiguredExtraFormTypesApiUrl,a={name:e,configuration:JSON.stringify(o,null,2)};this.$http.post(s,a).then(function(t){this.modal.type="success",this.modal.content="This field has been successfully saved",this.$store.commit("addConfiguredType",t.body)},function(t){this.modal.type="put",this.modal.content=t.body+". Do you want to update the configuration ?"})},updateConfiguredType:function(t){var e=this.createExtraFormRawRecursively([t]),i=this.transformFieldToType(e[t.name]),o=this.$store.getters.putConfiguredExtraForTypesApiUrl(t.name),s={configuration:JSON.stringify(i,null,2)};this.$http.put(o,s).then(function(t){this.modal.type="success",this.modal.content="This field has been successfully updated",this.$store.commit("updateConfiguredType",t.body)},function(t){this.modal.type="error",this.modal.content=t.body})},removeField:function(t){this.$emit("removed",t)},setActiveClass:function(t){this.$emit("active-class",t)}}};
