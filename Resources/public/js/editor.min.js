...
var baseExtraFormType={template:'<div><button class="extra-btn" @click="createField(type)" :class="type.name" type="button"><i :class="getFontAwsomeIconClass(type.icon)" aria-hidden="true"></i>{{ type.name }}</button></div>',props:["type"],mixins:[fontAwesomeIconMixin],methods:{createField:function(e){this.$emit("created",e)}}};
...
function triggerVueEditor(e,t,s){Vue.component("Multiselect",VueMultiselect["default"]),Vue.use(VueResource);var r=new Vuex.Store({state:{configuration:s,formProperties:t,configuredTypes:[],baseTypes:[],apiCache:{}},getters:extraFormEditorGetters,mutations:extraFormEditorMutations,actions:extraFormEditorActions});new Vue({el:e,store:r,data:{fields:[]},beforeCreate:function(){r.dispatch("setBaseExtraFormTypes",this.$http),r.dispatch("setConfiguredExtraFormTypes",this.$http)},methods:{updateFields:function(e){this.$set(this,"fields",e)}}})}
