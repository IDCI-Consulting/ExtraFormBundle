...
var editorAdvancedField={template:'<div :class="activeClass" @click="setActiveClass(index)"><button @click.prevent="removeField(index)" aria-label="Close" class="close"><span aria-hidden="true">×</span></button><i class="fa fa-arrows-alt"></i><i v-if="configuredTypesEditionAllowed" title="Save this field as a configured type" @click="openSaveModal" class="fa fa-floppy-o"></i><strong>{{ field.extra_form_type }}</strong><i :class="getFontAwsomeIconClass(field.icon, field.extra_form_type)" aria-hidden="true"></i><br><span>Name:<input class="field-name-input" type="text" v-model="field.name" pattern="/^([a-z][0-9])+$/" /></span><modal v-if="modal.show"><h3 slot="header">Save this configured field<button @click="closeSaveModal" type="button" class="close" aria-label="Close">&times;</button></h3><div v-if="modal.done" slot="body"><div v-html="modal.done"></div></div><div v-else slot="body"><div><i :class="getFontAwsomeIconClass(field.icon)" aria-hidden="true"></i></div><div>Type: <strong>{{ field.extra_form_type }}</strong></div><div>Name: <strong>{{ field.name }}</strong></div><div>Tags: <input v-model="tags" type="text"></div></div><div slot="footer"><div v-if="modal.type == \'save\'"><button @click="saveConfiguredType(field)" type="button" class="extra-btn" aria-label="Save">Save</button></div><div v-if="modal.type == \'put\'"><button @click="updateConfiguredType(field)" type="button" class="extra-btn" aria-label="Save">Update</button></div></div></div></modal></div>',props:["field","index"],data:function(){return{modal:null,tags:""}},created:function(){this.setInitialSaveModal(),this.tags=this.$store.getters.getConfiguredExtraFormTypesTags.filter(function(e){return 0!==e.indexOf("-")}).join(",")},computed:{activeClass:function(){var e=this.field.active?"active":"inactive";return"field "+e},configuredTypesEditionAllowed:function(){return this.$store.getters.configuredTypesEditionAllowed}},mixins:[fontAwesomeIconMixin,rawMixin],methods:{setInitialSaveModal:function(){this.modal={show:!1,type:"save"}},openSaveModal:function(){this.modal.show=!0},closeSaveModal:function(){this.modal.show=!1,this.setInitialSaveModal()},transformFieldToType:function(e){var t=JSON.parse(JSON.stringify(e));return t.extra_form_options=t.options,delete t.options,t.extra_form_constraints=t.constraints,delete t.constraints,t.form_type=t.extra_form_type,delete t.extra_form_type,t},saveConfiguredType:function(e){var t=e.name,i=this.createExtraFormRawRecursively([e]),o=this.transformFieldToType(i[e.name]),s=this.$store.getters.postConfiguredExtraFormTypesApiUrl,a={name:t,tags:this.tags,configuration:JSON.stringify(o,null,2)};this.$http.post(s,a).then(function(e){this.modal.type="success",this.modal.done="This field has been successfully saved",this.$store.commit("addConfiguredType",e.body)},function(e){this.modal.type="put",this.modal.done=e.body+". Do you want to update the configuration ?"})},updateConfiguredType:function(e){var t=this.createExtraFormRawRecursively([e]),i=this.transformFieldToType(t[e.name]),o=this.$store.getters.putConfiguredExtraForTypesApiUrl(e.name),s={configuration:JSON.stringify(i,null,2)};""!==this.tags&&(s.tags=this.tags),this.$http.put(o,s).then(function(e){this.modal.type="success",this.modal.done="This field has been successfully updated",this.$store.commit("updateConfiguredType",e.body)},function(e){this.modal.done=e.body})},removeField:function(e){this.$emit("removed",e)},setActiveClass:function(e){this.$emit("active-class",e)}}};
