...
var optionMixin={props:["option","name","value"],data:function(){return{data:this.option.options.data}},created:function(){"undefined"!=typeof this.value&&(this.data=this.value,"string"!=typeof this.value&&(this.data=JSON.stringify(this.value)))},watch:{value:{handler:function(t){this.data=t,"string"!=typeof t&&(this.data=JSON.stringify(t))}}},methods:{updateOption:function(t){this.data=t,this.$emit("changed",{name:this.name,value:t})},setJsonAttemptClass:function(t){if(0===t.indexOf("{")||0===t.indexOf("["))try{JSON.parse(t),this.classes="fa fa-check success feedback"}catch(a){this.classes="fa fa-exclamation-circle warning feedback"}else this.classes=""}}};
...
var choiceOption={template:'<div class="form-group"><label :for="name">{{ name }}</label><select class="form-control" :required="option.options.required" v-model="data" :name="name" @change="updateOption($event.target.value)"><option :value="key" v-for="(choice, key) in option.options.choices">{{ choice }}</option></select></div>',mixins:[optionMixin],watch:{value:{handler:function(e){this.data=e},deep:!0}}};
