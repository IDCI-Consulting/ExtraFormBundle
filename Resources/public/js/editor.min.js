...
window.extraQuery=jQuery.noConflict(!0);
...
function filterObject(e,t){var r={};for(var a in e)e.hasOwnProperty(a)&&t(e[a])&&(r[a]=e[a]);return r}function sortObject(e,t,r){const a={};"undefined"==typeof r&&(r=!0),"undefined"==typeof t&&(t=[]);for(var n=0,o=t.length;n<o;n++){var d=t[n];a[d]=e[d]}if(r)Object.keys(e).sort().forEach(function(r){t.indexOf(r)===-1&&(a[r]=e[r])});else for(var d in e)e.hasOwnProperty(d)&&t.indexOf(d)===-1&&(a[d]=e[d]);return a}function generateUniqueId(){return Math.random().toString(36).substr(2,9)}function getLastKey(e){var t=Object.keys(e),r=t.length-1;return t[r]}function hashCode(e){var t,r=0;if(0===e.length)return r;for(var a=0;a<e.length;a++)t=e.charCodeAt(a),r=(r<<5)-r+t,r|=0;return r}function createAttributeMapObject(e){for(var t,r=e.attributes,a={},n=0,o=r.length;n<o;n++)t=r[n],a[t.nodeName]=t.nodeValue;return a.value=e.value,a}function createBootstrapModal(e,t,r,a,n,o){var d=o?o:"";return'<div id="'+t+"-"+e+'" class="editor-modal modal fade '+r+" "+t+'"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="close" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 class="modal-title">'+a+'</h4></div><div class="modal-body">'+n+'</div><div class="modal-footer">'+d+"</div></div></div></div>"}
var optionMixin={props:["option","name","value"],data:function(){return{data:this.option.options.data}},created:function(){"undefined"!=typeof this.value&&(this.data=this.value,"string"!=typeof this.value&&(this.data=JSON.stringify(this.value)))},watch:{value:{handler:function(t){"undefined"!=typeof t&&(this.data=t,"string"!=typeof t&&(this.data=JSON.stringify(t)))}}},methods:{updateOption:function(t){"undefined"!=typeof t&&(this.data=t,this.$emit("changed",{name:this.name,value:t}))},setJsonAttemptClass:function(t){if(0===t.indexOf("{")||0===t.indexOf("["))try{JSON.parse(t),this.classes="fa fa-check success feedback"}catch(a){this.classes="fa fa-exclamation-circle warning feedback"}else this.classes=""}}};
...
var textareaOption={template:'<div class="form-group"><label :for="name">{{ name }}</label><textarea class="form-control" :required="option.options.required" :value="data" @input="onOptionValueChanged($event.target.value)" :name="name"></textarea><i :class="classes" aria-hidden="true"></i></div>',data:function(){return{classes:""}},mixins:[optionMixin],methods:{onOptionValueChanged:function(e){this.updateOption(e),this.setJsonAttemptClass(e)}}};
...
var textOption={template:'<div class="form-group"><label :for="name">{{ name }}</label><input class="form-control" :required="option.options.required" :value="data" @input="onOptionValueChanged($event.target.value)" type="text" :name="name"><i :class="classes" aria-hidden="true"></i></div>',data:function(){return{classes:""}},mixins:[optionMixin],methods:{onOptionValueChanged:function(t){this.updateOption(t),this.setJsonAttemptClass(t)}}};
...
Vue.component("form-editor-raw",{template:'<div><textarea :data-textarea-id="textarea.id" v-model="raw" :name="textarea.name" style="width: 100%; height: 300px;"></textarea><button class="close-modal" @click.prevent="generateFields">Fill the visual mode form fields from this json</button></div>',props:["fields"],data:function(){return{raw:"",textarea:this.$store.state.formProperties}},mixins:[rawMixin],created:function(){try{""!==this.textarea.value&&(this.raw=this.textarea.value,this.generateFields())}catch(e){}},watch:{fields:{handler:function(e){this.raw=this.generateRaw(e),this.updateInitialTextareaValue()},deep:!0}},methods:{updateInitialTextareaValue:function(){this.textarea.id&&(document.getElementById(this.textarea.id).value=this.raw)},generateFields:function(){var e=[];try{var t=JSON.parse(this.raw);e=this.createFieldsRecursively(t),this.$emit("generated",e)}catch(a){}},generateRaw:function(e){var t=this.createExtraFormRawRecursively(e);return JSON.stringify(t,null,4)}}});
...
window.loadExtraFormEditors=function(){extraQuery("textarea.extra-form-editor").each(function(e){function a(){var e=extraQuery(".editor-simple");e.on("click",".field-options > label",function(e){e.stopImmediatePropagation(),extraQuery(this).parent().toggleClass("show")}),e.on("click",".field-constraint-options > label",function(e){e.stopImmediatePropagation(),extraQuery(this).parent().toggleClass("show")})}function o(){extraQuery(document).on("change",'.extra-form-inputs-required input[required="required"]',function(){extraQuery(this).val()?extraQuery(this).css({"border-color":"#cccccc","background-color":"#ffffff"}):extraQuery(this).css({"border-color":"#c9302c","background-color":"#f3d9d9"})})}function t(e,a){extraQuery(document).on("click","button.trigger-"+e+"-"+a,function(o){o.preventDefault();var t=extraQuery("#"+e+"-"+a);"overview-modal"!==e&&(t=t.first()),t.modal("show")})}function r(e){var a="."+e+" .modal-body button.close-modal, ."+e+" .modal-footer > button.close-modal, ."+e+" .modal-header > button.close";extraQuery(document).on("click",a,function(e){e.preventDefault(),extraQuery(this).closest(".modal").modal("hide")})}function i(a){function o(o){var t=extraQuery("#raw-mode-modal-"+e+" textarea").first().val(),r=extraQuery.ajax({url:a,method:"POST",data:{overview:{configuration:t}}});r.done(function(e){return o({success:!0,data:e})}),r.fail(function(e){return o({success:!1,data:e})})}extraQuery(document).on("click","button.trigger-overview-modal-"+e,function(){setTimeout(function(){o(function(a){a.success?extraQuery("#overview-modal-"+e+" .modal-body").replaceWith('<div class="modal-body">'+a.data+"</div>"):extraQuery("#overview-modal-"+e+" .modal-body").replaceWith('<div class="modal-body"><div>'+a.data.responseText+"</div></div>")})},800)})}function d(){extraQuery(document).on("click","#overview-modal-"+e+" form button[type='submit']",function(a){a.preventDefault(),l(e);var o=extraQuery(this).closest("form");setTimeout(function(){c(o,function(a){a.success?a.data&&extraQuery("#overview-modal-"+e+" .modal-body").replaceWith('<div class="modal-body">'+a.data+"</div>"):extraQuery("#overview-modal-"+e+" .modal-body").replaceWith('<div class="modal-body"><div>'+a.data.responseText+"</div></div>")})},800)})}function n(){extraQuery(document).on("hidden.bs.modal","#overview-modal-"+e,function(){l(e)})}function l(){extraQuery("#overview-modal-"+e+" .modal-body").replaceWith('<div class="modal-body"><div style="text-align: center;"><i class="fa fa-cog fa-spin fa-3x fa-fw"></i></div></div>')}function c(e,a){var o=extraQuery.ajax({url:e.attr("action"),method:e.attr("method"),data:e.serialize()});o.done(function(e){return a({success:!0,data:e})}),o.fail(function(e){return a({success:!1,data:e})})}function s(){return createBootstrapModal(e,"raw-mode-modal","modal-fullscreen","Editor in raw mode",'<form-editor-raw :fields="fields" @generated="updateFields"></form-editor-raw><br>')}function u(){return createBootstrapModal(e,"simple-visual-mode-modal","modal-fullscreen","Simple visual mode",'<form-editor-simple :fields="fields"></form-editor-simple>',"<em>All your changes are automatically saved</em>")}function m(){return createBootstrapModal(e,"advanced-visual-mode-modal","modal-fullscreen","Advanced visual mode",'<form-editor-advanced :fields="fields"></form-editor-advanced>','<button class="btn btn-primary trigger-overview-modal-'+e+'">Display the overview <i class="fa fa-eye"></i></button><em>All your changes are automatically saved</em>')}function f(){return createBootstrapModal(e,"overview-modal","extra-form-inputs-required","Overview",'<div style="text-align: center;"><i class="fa fa-cog fa-spin fa-3x fa-fw"></i></div>')}var v="editorComponent"+e;if(!document.getElementById(v)){var y=createAttributeMapObject(this),p=y["data-available-modes"].split("__"),x=window[y["data-configuration-variable"]],b=s(e),g='<button class="trigger-raw-mode-modal-'+e+'">Raw mode</button>',w="",h="",Q="",k="",E="";p.indexOf("simple")>-1&&(w=u(e),h='<button class="trigger-simple-visual-mode-modal-'+e+'">Simple visual mode</button>'),p.indexOf("advanced")>-1&&(Q=m(e),k=f(e),E='<button class="trigger-advanced-visual-mode-modal-'+e+'">Advanced visual mode</button>'),extraQuery(this).after('<div class="modal-buttons">'+h+" "+E+" "+g+"</div>");var _=extraQuery("body");_.append('<div id="'+v+'">'+b+w+Q+k+"</div>"),this.style.display="none";var A=["simple-visual-mode-modal","advanced-visual-mode-modal","raw-mode-modal","overview-modal"];A.forEach(function(a){t(a,e)}),A.forEach(function(e){r(e)}),i(x.extra_form_editor_overview_url,e),n(e),d(e),o(),a(),triggerVueEditor("#"+v,y,x)}})};